...
public class SequenceFunctionRewrite implements ExpressionPatternRuleFactory {
    ...
    public List<ExpressionPatternMatcher<? extends Expression>> buildRules() {
        return ImmutableList.of(
                ...
                matchesType(SequenceHourUnit.class).then(sequenceHourUnit -> new ArrayRangeHourUnit(
                        sequenceHourUnit.child(0), new HoursAdd(sequenceHourUnit.child(1),
                                new IntegerLiteral(1)), sequenceHourUnit.child(2))),
                matchesType(SequenceMinuteUnit.class).then(sequenceMinuteUnit -> new ArrayRangeMinuteUnit(
                                sequenceMinuteUnit.child(0), new MinutesAdd(sequenceMinuteUnit.child(1),
                                        new IntegerLiteral(1)), sequenceMinuteUnit.child(2))),
                matchesType(SequenceSecondUnit.class).then(sequenceSecondUnit -> new ArrayRangeSecondUnit(
                                sequenceSecondUnit.child(0), new SecondsAdd(sequenceSecondUnit.child(1),
                                        new IntegerLiteral(1)),
                                sequenceSecondUnit.child(2))),
                matchesType(SequenceYearUnit.class).then(sequenceYearUnit -> new ArrayRangeYearUnit(
                                sequenceYearUnit.child(0), new YearsAdd(sequenceYearUnit.child(1),
                                        new IntegerLiteral(1)),
                                sequenceYearUnit.child(2))),
                matchesType(SequenceWeekUnit.class).then(sequenceWeekUnit -> new ArrayRangeWeekUnit(
                                sequenceWeekUnit.child(0), new WeeksAdd(sequenceWeekUnit.child(1),
                                        new IntegerLiteral(1)),
                                sequenceWeekUnit.child(2))),
                matchesType(SequenceMonthUnit.class).then(sequenceMonthUnit -> new ArrayRangeMonthUnit(
                                sequenceMonthUnit.child(0), new MonthsAdd(sequenceMonthUnit.child(1),
                                        new IntegerLiteral(1)), sequenceMonthUnit.child(2))),
                matchesType(SequenceDayUnit.class).then(sequenceDayUnit -> new ArrayRangeDayUnit(
                                sequenceDayUnit.child(0), new DaysAdd(sequenceDayUnit.child(1),
                                        new IntegerLiteral(1)), sequenceDayUnit.child(2)))
