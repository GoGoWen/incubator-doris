...
public class ThriftHMSCachedClient implements HMSCachedClient {
    ...
    public void createTable(TableMetadata tbl, boolean ignoreIfExists) {
        if (tableExists(tbl.getDbName(), tbl.getTableName())) {
            throw new HMSClientException(String.format("Table '%s' has existed in '%s'.",
            ...
            try {
                ...
                if (tbl instanceof HiveTableMetadata) {
                    ...
                    if (!dvs.isEmpty()) {
                        ...
                        client.client.createTableWithConstraints(hiveTable, null, null, null, null,
                                dvs, null);
                    } else {
    ...
    public Map<String, String> getDefaultColumnValues(String dbName, String tblName) {
        ...
        try (ThriftHMSClient client = getClient()) {
            try {
                ...
                List<SQLDefaultConstraint> dvcs = ImmutableList.of();
    ...
    public void updateTableStatistics(
        ...
        try (ThriftHMSClient client = getClient()) {
            ///  isFromView is false, updateTableStatistics only used in insert data into hive table
