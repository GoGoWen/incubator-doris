...
public class HiveTypeToDorisTypeTest {
    ...
    public void testCharType() {
        Assert.assertEquals(ScalarType.createType(PrimitiveType.CHAR, 10, 0, 0), hiveTypeToDorisType("char(10)", 0));
        Assert.assertEquals(ScalarType.createType(PrimitiveType.CHAR), hiveTypeToDorisType("char", 0));
    }

    @Test
    public void testVarcharType() {
        Assert.assertEquals(ScalarType.createType(PrimitiveType.VARCHAR, 20, 0, 0), hiveTypeToDorisType("varchar(20)", 0));
        Assert.assertEquals(ScalarType.createType(PrimitiveType.VARCHAR), hiveTypeToDorisType("varchar", 0));
    }

    @Test
    public void testDecimalType() {
        Assert.assertEquals(ScalarType.createDecimalV3Type(10, 2), hiveTypeToDorisType("decimal(10,2)", 0));
        Assert.assertEquals(ScalarType.createDecimalV3Type(HiveMetaStoreClientHelper.DEFAULT_PRECISION, HiveMetaStoreClientHelper.DEFAULT_SCALE),
    ...
    private Type hiveTypeToDorisType(String hiveType, int timeScale) {
        return HiveMetaStoreClientHelper.hiveTypeToDorisType(hiveType, timeScale);
    }
}
