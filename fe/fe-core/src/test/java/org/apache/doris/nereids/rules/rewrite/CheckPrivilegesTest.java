...
class CheckPrivilegesTest {
    ...
    void testNormalColumns() {
        ...
        Set<Slot> requiredSlots = new LinkedHashSet<>(outputs);

        Set<String> result = checkPrivileges.computeUsedColumns(mockPlan, requiredSlots);
    ...
    void testNoPermissionColumn() {
        ...
        AnalysisException exception = Assertions.assertThrows(AnalysisException.class,
                () -> checkPrivileges.computeUsedColumns(mockPlan, requiredSlots));
    ...
    void testNonExistColumn() {
        ...
        Plan mockPlan = createMockPlan(outputs);
        Set<Slot> requiredSlots = new LinkedHashSet<>(Arrays.asList(
                existCol, new SlotReference("non_exist_col", IntegerType.INSTANCE)
        ));
    ...
    private SlotReference createSlotRef(String name, boolean hasPerm, boolean isHidden) {
        ...
        if (isHidden) {
            Mockito.when(slot.getColumn()).thenReturn(Optional.of(mock));
        } else {
            Mockito.when(slot.getColumn()).thenReturn(Optional.of(mock));
        }
        return slot;
