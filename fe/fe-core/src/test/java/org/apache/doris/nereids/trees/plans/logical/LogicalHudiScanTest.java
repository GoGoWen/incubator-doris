 ...
 * through all with* methods to prevent type slicing issues.
 */
public class LogicalHudiScanTest {
    ...
    public void testWithOperativeSlotsReturnsLogicalHudiScan() {
        LogicalHudiScan hudiScan = new LogicalHudiScan(
                relationId, mockTable, qualifier,
                Optional.empty(), Optional.empty());

        LogicalHudiScan result = hudiScan.withOperativeSlots(operativeSlots);
    ...
    /**
     * Test that withConjuncts preserves operativeSlots.
     */
    @Test
    public void testWithConjunctsPreservesOperativeSlots() {
        LogicalHudiScan hudiScan = createHudiScanWithOperativeSlots();
        Set<Expression> newConjuncts = Sets.newHashSet();

        LogicalHudiScan result = hudiScan.withConjuncts(newConjuncts);
    ...
    public void testEmptyOperativeSlots() {
        ...
        LogicalHudiScan result = hudiScan.withOperativeSlots(ImmutableList.of());
    ...
    public void testWithOperativeSlotsPreservesScanParams() {
        ...
        Assertions.assertEquals(hudiScan.getIncrementalRelation(), result.getIncrementalRelation());
    }
    ...
    private LogicalHudiScan createHudiScanWithOperativeSlots() {
        LogicalHudiScan hudiScan = new LogicalHudiScan(
                relationId, mockTable, qualifier,
                Optional.empty(), Optional.empty());
        return hudiScan.withOperativeSlots(operativeSlots);
    }

    /**
     * Test the backward compatible constructor (without operativeSlots parameter).
     * This constructor delegates to the main constructor with empty operativeSlots.
     * Covers lines 96-98.
     */
    @Test
    public void testBackwardCompatibleConstructor() {
        // Use the helper class to access the protected constructor
        LogicalHudiScan hudiScan = TestableLogicalHudiScan.createWithBackwardCompatibleConstructor(
                relationId, mockTable, qualifier,
                Optional.empty(), Optional.empty(),
        ...
        Assertions.assertTrue(hudiScan.getOperativeSlots().isEmpty(),
                "Backward compatible constructor should initialize with empty operativeSlots");
        Assertions.assertEquals(relationId, hudiScan.getRelationId());
    }

    /**
     * Test that withScanParams preserves operativeSlots when scanParams is null.
     * Covers line 258.
     */
    @Test
    public void testWithScanParamsNullPreservesOperativeSlots() {
        LogicalHudiScan hudiScan = createHudiScanWithOperativeSlots();

        // withScanParams with null scanParams should preserve operativeSlots
        // When scanParams is null, the incremental read block is skipped and
        // a new LogicalHudiScan is returned with preserved operativeSlots
        LogicalHudiScan result = hudiScan.withScanParams(mockHmsTable, null);
    ...
    /**
     * Helper class to access protected constructor for testing.
     */
    static class TestableLogicalHudiScan extends LogicalHudiScan {

        protected TestableLogicalHudiScan(RelationId id, ExternalTable table, List<String> qualifier,
        ...
        public static LogicalHudiScan createWithBackwardCompatibleConstructor(
                RelationId id, ExternalTable table, List<String> qualifier,
                Optional<GroupExpression> groupExpression, Optional<LogicalProperties> logicalProperties,
                Set<Expression> conjuncts, SelectedPartitions selectedPartitions,
                Optional<org.apache.doris.nereids.trees.TableSample> tableSample,
                Optional<org.apache.doris.analysis.TableSnapshot> tableSnapshot,
                Optional<org.apache.doris.analysis.TableScanParams> scanParams,
                Optional<org.apache.doris.datasource.hudi.source.IncrementalRelation> incrementalRelation) {
            return new TestableLogicalHudiScan(id, table, qualifier, groupExpression, logicalProperties,
                    conjuncts, selectedPartitions, tableSample, tableSnapshot, scanParams, incrementalRelation);
        }
    }
}
