...
class PartitionValuesRewriteTest implements MemoPatternMatchSupported {
    ...
    void testCannotApplyToNonPartitionValuesFunction() {
        ...
        LogicalTVFRelation tvfRelation = new LogicalTVFRelation(new RelationId(1),
                nonPartitionValuesTVF, Optional.empty(), Optional.empty());
        ...
        new MockUp<LogicalTVFRelation>() {
            @Mock
            public org.apache.doris.nereids.trees.expressions.functions.table.TableValuedFunction getFunction() {
                return new org.apache.doris.nereids.trees.expressions.functions.table.TableValuedFunction("other_tvf",
                        new Properties(Maps.newHashMap())) {
                    @Override
                    protected org.apache.doris.tablefunction.TableValuedFunctionIf toCatalogFunction() {
                        // Return a mock implementation - this is just for testing non-PartitionValues functions
                        return new org.apache.doris.tablefunction.TableValuedFunctionIf() {
                            @Override
                            ...
                            @Override
                            public org.apache.doris.planner.ScanNode getScanNode(org.apache.doris.planner.PlanNodeId nodeId,
                                    org.apache.doris.analysis.TupleDescriptor desc, org.apache.doris.qe.SessionVariable sessionVariable) {
    ...
    void testPartitionValuesWithExactThresholdPartitionCount() {
        ...
        new Expectations() {{
                    ...
                    mockCache.getPartitionValues(TEST_DB, TEST_TABLE, (List<Type>) any);
                    result = createMockHivePartitionValues();
                    minTimes = 0;

                    mockCache.getPartitionValuesByFilter(TEST_DB, TEST_TABLE, anyString, (List<String>) any, (List<Type>) any);
    ...
    void testMultiplePartitionColumns() {
        ...
        new Expectations() {{
                ...
                mockCache.getPartitionValuesByFilter(TEST_DB, TEST_TABLE, anyString,
    ...
    void testTypeCoercionInConstantExpressions() {
        ...
        new MockUp<org.apache.doris.common.util.HMSPartitionsUtil>() {
            ...
            public List<List<NamedExpression>> buildConstantExpressionsFromPartitions(
                    java.util.Collection<PartitionItem> selectedPartitions,
                    List<Column> partitionColumns,
                    Map<String, org.apache.doris.nereids.types.DataType> nameToType,
    ...
    private LogicalTVFRelation createMockTVFRelation() {
        return new LogicalTVFRelation(new RelationId(1), mockPartitionValues,
    ...
    private void setupFilteredPartitionMocks(List<String> expectedFilters) {
        new Expectations() {{
                mockCache.getPartitionValuesByFilter(TEST_DB, TEST_TABLE,
                        (String) any, (List<String>) any, (List<Type>) any);
                result = createMockSelectedPartitions();
                minTimes = 0;
            }};
    }
    ...
    private void setupViewBasedTableMocks() {
        new Expectations() {{
                ...
                mockCache.getPartitionValuesByFilterFromView(TEST_DB, TEST_TABLE,
                        (String) any, (List<String>) any, (List<Type>) any);
                result = createMockSelectedPartitions();
                minTimes = 0;
            }};
    ...
    private void setupLargePartitionCountMocks() {
        new Expectations() {{
                ...
                mockCache.getPartitionValuesWithoutCache(TEST_DB, TEST_TABLE, (List<Type>) any);
                result = createMockHivePartitionValues();
                minTimes = 0;
            }};
    }
