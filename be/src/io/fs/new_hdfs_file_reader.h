...
class NewHdfsFileReader : public FileReader {
public:
    NewHdfsFileReader(Path path, const std::string& name_node, FileSystemSPtr fs,
        std::unique_ptr<ExclusiveHdfsFileHandle> handle, RuntimeProfile* profile);

    ~NewHdfsFileReader() override;
...
private:
    ...
    FileSystemSPtr _fs;
    std::unique_ptr<ExclusiveHdfsFileHandle> _handle = nullptr;
