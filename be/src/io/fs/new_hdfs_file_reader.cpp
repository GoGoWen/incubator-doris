...
NewHdfsFileReader::NewHdfsFileReader(Path path, const std::string& name_node, FileSystemSPtr fs,
    std::unique_ptr<ExclusiveHdfsFileHandle> handle, RuntimeProfile* profile)
        : _path(std::move(path)),
...
NewHdfsFileReader::~NewHdfsFileReader() {
    static_cast<void>(close());
}

Status NewHdfsFileReader::close() {
    bool expected = false;
...
#ifdef USE_HADOOP_HDFS
Status NewHdfsFileReader::read_at_impl(size_t offset, Slice result, size_t* bytes_read,
...
void NewHdfsFileReader::_collect_profile_before_close() {
}
