...
public:
    ...
    Status execute_impl(FunctionContext* context, Block& block, const ColumnNumbers& arguments,
        ...
        if (json_data_column->is_nullable()) {
            const auto* nullable = check_and_get_column<ColumnNullable>(json_data_column);
            json_data_column = nullable->get_nested_column_ptr();
            json_null_map = &nullable->get_null_map_data();
        }
        const auto* json_col = check_and_get_column<ColumnString>(json_data_column);
        if (!json_col) {
        ...
        if (path_column->is_nullable()) {
            const auto* nullable = check_and_get_column<ColumnNullable>(path_column);
            path_column = nullable->get_nested_column_ptr();
            path_null_map = &nullable->get_null_map_data();
        }
        const auto* path_col = check_and_get_column<ColumnString>(path_column);
        if (!path_col) {
