...
struct HashImpl {
    ...
    static Status empty_apply(IColumn& icolumn, size_t input_rows_count) {
        ColumnVector<Int32>& vec_to = assert_cast<ColumnVector<Int32>&>(icolumn);
        vec_to.get_data().assign(input_rows_count, static_cast<Int32>(emtpy_value));
        return Status::OK();
    }
    ...
    static Status execute(const IDataType* type, const IColumn* column, size_t input_rows_count,
                          IColumn& col_to) {
        auto& to_column = assert_cast<ColumnVector<Int32>&>(col_to);
        if constexpr (first) {
            to_column.insert_many_defaults(input_rows_count);
        }
        auto& col_to_data = to_column.get_data();
        ...
        } else if (const auto* col_from_const = check_and_get_column<ColumnInt32>(column)) {
            for (size_t i = 0; i < input_rows_count; ++i) {
                col_to_data[i] = col_from_const->get_element(i);
            }
        } else {
            DCHECK(false);
            return Status::NotSupported("Illegal column {} of argument of function {}",
